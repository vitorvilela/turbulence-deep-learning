# Large Eddy Artificial Intelligence Simulation (LEAIS) - Python 3
# Author: Vitor Vilela
# Created at: 28/09/2017
# Last modification at: 07/03/2018
...
# Time Step
DT = 0.001 #DELTA_FINE*DELTA_FINE # 1.52587890625E-005 for FINE_MESH=256
#DT = DELTA_FINE # 0.00390625 for FINE_MESH=256
...
EXPORT_AI = True
#PRINT_AI_AT = ST/50
#PRINT_AI_AT = 100*DT # 0.390625
PRINT_AI_AT = 100*DT #10000*DT # 1.52587890625E-001
...
# Outputs
# Coarse Velocity, Filtered at t-2, t-1 and t
U, V = [np.zeros((3, COARSE_MESH+2, COARSE_MESH+2), dtype=np.float64) for _ in range(2)]

# Coarse Cross-Velocity, Filtered
UU, VV, UV = [np.zeros((3, COARSE_MESH+2, COARSE_MESH+2), dtype=np.float64) for _ in range(3)]
...
# Fill past filtered velocity fields
def filtered_fill_pastTimes():
  
  for I in range(F[COARSE]['GL'], F[COARSE]['GR']+1):
    for J in range(F[COARSE]['GB'], F[COARSE]['GT']+1):
      U[0,I,J] = U[1,I,J]
      U[1,I,J] = U[2,I,J]
      V[0,I,J] = V[1,I,J]
      V[1,I,J] = V[2,I,J] 
...
def filtered_velocity():
  ...
  for I in range(F[COARSE]['L'], F[COARSE]['R']+1):
    for J in range(F[COARSE]['B'], F[COARSE]['T']+1): 
      ...
      for i in range(1, MR):
        for j in range(1, MR):
          U[2,I,J] += (1./pow(MR, 2.))*(u[EC['C'], (I-1)*MR+i, (J-1)*MR+j])
          V[2,I,J] += (1./pow(MR, 2.))*(v[EC['C'], (I-1)*MR+i, (J-1)*MR+j])
...
def filtered_crossVelocity():
  ...
  for I in range(F[COARSE]['L'], F[COARSE]['R']+1):
    for J in range(F[COARSE]['B'], F[COARSE]['T']+1):
      ...
      for i in range(1, MR):
        for j in range(1, MR):  
          UU[2,I,J] += (1./pow(MR, 2.))*(uu[(I-1)*MR+i, (J-1)*MR+j])
          VV[2,I,J] += (1./pow(MR, 2.))*(vv[(I-1)*MR+i, (J-1)*MR+j])
          UV[2,I,J] += (1./pow(MR, 2.))*(uv[(I-1)*MR+i, (J-1)*MR+j])
...
def flow_filteredStress():
  ...
  for i in range(F[COARSE]['L'], F[COARSE]['R']+1):
    for j in range(F[COARSE]['B'], F[COARSE]['T']+1):
      sxx_hat[i,j] = -UU[2,i,j] + 2.*NU*(u_hat[EC['C'],i+1,j]-u_hat[EC['C'],i,j])/DELTA_COARSE
      syy_hat[i,j] = -VV[2,i,j] + 2.*NU*(v_hat[EC['C'],i,j+1]-v_hat[EC['C'],i,j])/DELTA_COARSE
  
  # Shear stress
  for i in range(F[COARSE]['L'], F[COARSE]['R']+2):
    for j in range(F[COARSE]['B'], F[COARSE]['T']+2):
      sxy_hat[i,j] = -UV[2,i,j] + NU*(u_hat[EC['C'],i,j]-u_hat[EC['C'],i,j-1]+v_hat[EC['C'],i,j]-v_hat[EC['C'],i-1,j])/DELTA_COARSE
    ...
    for j in range(F[COARSE]['B'], F[COARSE]['T']+1): 
      line = '{:.8f}{}{:d}{}{:d}{}' \
	     ...
	     '{:.16f}{}{:.16f}{}' \
             '{:.16f}{}{:.16f}{}' \
	     '{:.16f}{}{:.16f}{}{:.16f}{}{:.16f}{}{:.16f}{}{:.16f}{}{:.16f}{}{:.16f}{}{:.16f}{}{:.16f}{}' \
	     '{:.16f}{}{:.16f}{}{:.16f}{}{:.16f}{}{:.16f}{}'.format(t, ',', i, ',', j, ',',      
             U[0,i,j], ',', V[0,i,j], ',',
             U[1,i,j], ',', V[1,i,j], ',',
             U[2,i-1,j], ',', U[2,i,j-1], ',', U[2,i,j], ',', U[2,i,j+1], ',', U[2,i+1,j], ',', V[2,i-1,j], ',', V[2,i,j-1], ',', V[2,i,j], ',', V[2,i,j+1], ',', V[2,i+1,j], ',',
             UU[0,i,j], ',', VV[0,i,j], ',',
             UU[1,i,j], ',', VV[1,i,j], ',',
             UU[2,i-1,j], ',', UU[2,i,j-1], ',', UU[2,i,j], ',', UU[2,i,j+1], ',', UU[2,i+1,j], ',', VV[2,i-1,j], ',', VV[2,i,j-1], ',', VV[2,i,j], ',', VV[2,i,j+1], ',', VV[2,i+1,j], ',', 
             UV[2,i-1,j], ',', UV[2,i,j-1], ',', UV[2,i,j], ',', UV[2,i,j+1], ',', UV[2,i+1,j], '\n')
      ai_file.write(line)  
...
def field_print(field_file):
  for i in range(F[COARSE]['L'], F[COARSE]['R']+1):
    for j in range(F[COARSE]['B'], F[COARSE]['T']+1):
      field_file.write('{:d}{}{:d}{}{:.16f}{}{:.16f}{}'.format(i, ',', j, ',', U[2,i,j], ',', V[2,i,j], '\n'))
...
# TODO Do not print header into the file
if EXPORT_AI:
...
while t <= ST:
  ...
  if t % PRINT_AT < DT:     
    ...
    if PRINT_COMPARE:
      ...
      # Check out what field has been printed!
