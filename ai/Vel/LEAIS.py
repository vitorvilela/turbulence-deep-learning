...
# Last modification at: 02/10/2017
# Accumulated Man Hours: 8
#
...
# Coarse simulation with fine simulation
RUN_FILTERED = False

# Fine mesh is the filtered mesh running model
RUN_MODEL = False

# Print u, v field for comparison 
RUN_COMPARE = False


# Faces Dictionary (GHOST LEFT, LEFT, GHOST RIGHT, RIGHT, ..., BOTTOM, TOP)
F = []
...
def flow_ml_stress():
  ...
  for i in range(F[FINE]['L'], F[FINE]['R']+2):
    for j in range(F[FINE]['B'], F[FINE]['T']+2):
      ...
      # Shear stress
      sxy[i,j] = -uv_infer + NU*(u[EC['C'],i,j]-u[EC['C'],i,j-1]+v[EC['C'],i,j]-v[EC['C'],i-1,j])/DELTA_FINE
...
def ai_print(t, ai_file):
  ...
  for i in range(F[COARSE]['L'], F[COARSE]['R']+1):
    for j in range(F[COARSE]['B'], F[COARSE]['T']+1): 
      ai_file.write('{:.8f}{}{:d}{}{:d}{}{:.16f}{}{:.16f}{}{:.16f}{}{:.16f}{}{:.16f}{}'.format(t,',',i,',',j,',',U[i,j],',',V[i,j],',',UU[i,j],',',VV[i,j],',',UV[i,j],'\n'))  
